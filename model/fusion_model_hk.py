...
class EverythingAtOnceModel(nn.Module):
    ...
    def forward(self, video, audio, nframes, text, category, force_cross_modal=False):
        ...
        # Clustering loss
        video_mean = video_raw_embed.mean(dim=1)
        audio_mean = audio_raw_embed.mean(dim=1)
        text_mean = text_raw_embed.mean(dim=1)

        fushed = (video_mean + audio_mean + text_mean) / 3
        ...
        va = va.mean(dim=1)
        av = av.mean(dim=1)
        vav = torch.cat((va,av), dim=1).view(self.batch_size,-1)

        #origin_vav = torch.cat((video_mean,audio_mean), dim=1)
        #final_vav = torch.cat((vav,origin_vav),dim=1)
        vav= self.classifier1(vav)
        #vav= self.classifier1(final_vav)
        ...
        at = at.mean(dim=1)
        ta = ta.mean(dim=1)
        ata = torch.cat((at,ta), dim=1).view(self.batch_size,-1)
        #origin_ata = torch.cat((audio_mean,video_mean), dim=1)
        #final_ata = torch.cat((ata,origin_ata), dim=1)
        #ata= self.classifier2(final_ata)
        ...
        #origin_tvt = torch.cat((text_mean,video_mean), dim=1)
        #final_tvt = torch.cat((tvt,origin_tvt), dim=1)
        #tvt= self.classifier3(final_tvt)
        ...
        '''if self.use_cls_token:
            ...
            # a = (at + av) / 2
            # t = (ta + tv) / 2'''
        return vav, ata, tvt, video_mean, audio_mean, text_mean
